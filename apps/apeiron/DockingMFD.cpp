...
void DockingMFD::render(ImDrawList* dl, ImVec2 origin, ImVec2 size)
{
    ...
    // Helper: dark rounded pill behind a piece of text for readability over video.
    ...
    // When a texture is present fullBleed()==true so origin/size = full panel rect.
    // Compute the inner content area using MFDPanel layout constants.
    ImVec2 cOrig = origin;  // content-area origin
    ImVec2 cSize = size;    // content-area size
    if (m_texture != 0) {
        // Draw UV-cropped image filling the full panel (no letterboxing).
        ...
        dl->AddImage(m_texture,
                     origin,
                     { origin.x + size.x, origin.y + size.y },
                     uv0, uv1);

        // Shift content origin inward past title bar and button columns.
        cOrig = { origin.x + MFDPanel::kBtnW,
                  origin.y + MFDPanel::kTitleH };
        cSize = { size.x - 2.0f * MFDPanel::kBtnW,
    ...
    if (!m_hasTarget) {
        ...
        float tx = cOrig.x + (cSize.x - tsz.x) * 0.5f;
        float ty = cOrig.y + cSize.y * 0.5f - lh * 0.5f;
        if (m_texture != 0) pill(tx, ty, tsz);
        dl->AddText({ tx, ty }, kDim, msg);
        return;
    }
    ...
    // ---- Range + closure velocity ----
    {
        ...
        if (m_texture != 0) pill(cOrig.x + pad, y, rsz);
        dl->AddText({ cOrig.x + pad, y }, rngCol, buf);
        ...
        if (m_closureMs < 0.0f) velCol = kRed;
        std::snprintf(buf, sizeof(buf), "%+.2f m/s", m_closureMs);
        ImVec2 vsz = ImGui::CalcTextSize(buf);
        float vx = cOrig.x + cSize.x - vsz.x - pad;
        if (m_texture != 0) pill(vx, y, vsz);
        dl->AddText({ vx, y }, velCol, buf);
    }
    y += lh + pad;

    // ---- Lateral indicator ----
    const float footerH = 3.0f * (lh + pad);
    const float availH  = cSize.y - (y - cOrig.y) - footerH;
    const float sqSide  = std::max(20.0f, std::min(cSize.x - 2.0f * pad, availH));
    const ImVec2 sqTL   { cOrig.x + (cSize.x - sqSide) * 0.5f, y };
    const ImVec2 sqBR   { sqTL.x + sqSide, sqTL.y + sqSide };
    const float  cx     = sqTL.x + sqSide * 0.5f;
    const float  cy     = sqTL.y + sqSide * 0.5f;
    const float  hs     = sqSide * 0.5f;
    ...
    if (m_hasPort) {
        const float maxPY = m_threshAttErrDeg * 0.4f;
